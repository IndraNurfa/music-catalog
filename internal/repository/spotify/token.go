...
func (o *outbound) generateToken() error {
	...
	if err != nil {
		log.Error().Err(err).Msg("error create request for spotify token")
		return err
	}
	...
	if err != nil {
		log.Error().Err(err).Msg("error executing request body from spotify")
		return err
	}
	...
	if err != nil {
		log.Error().Err(err).Msg("error decode response body from spotify")
		return err
	}
